...
std::string print(ASMLine& line) {
  std::stringstream ss;
  ss << std::setw(4) << line.position.line << " | ";
  if(line.isOperation()) ss << print(line.getOperation());
  if(line.isLabel()) ss << "     |           | " << print(line.getLabel());
  if(line.isVoid()) ss << "     |           | ";
  ss << (line.comment.has_value() ? line.comment.value().comment : "");
  return ss.str();
}

std::string print(Operation& opr) {
  const int indent = 6;
  std::stringstream ss;
  ...
  for(auto arg_type : opr.format.operand) {
    ...
    if(arg_type == "imm") ss << std::string(imm_indent * indent, ' ') << print(opr.imm);
  }
  return ss.str();
}

std::string print(Label& lab) {
  std::stringstream ss;
