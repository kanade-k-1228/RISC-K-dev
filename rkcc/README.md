...
- [2. コンパイラの動作](#2-コンパイラの動作)
  ...
  - [2.3. シンボルテーブル生成](#23-シンボルテーブル生成)
    - [2.3.1. グローバル・シンボル](#231-グローバルシンボル)
    - [2.3.2. ローカル・シンボル](#232-ローカルシンボル)
...
プログラムのトップレベルで定義されたシンボルをまとめ、表を作ります。

| 名前 | 種類      | 型            | アドレス | ローカル | 関数の本体 |
| ---- | --------- | ------------- | -------- | -------- | ---------- |
| main | Func      | ()=>int       | ???      | local    | body       |
| sout | VarGlobal | int           | ???      | ---      | ---        |
| pair | Type      | {x:int,y:int} | ---      | ---      | ---        |

### 2.3.2. ローカル・シンボル

関数内の変数定義を探索し、ローカル変数のテーブルを作ります。
